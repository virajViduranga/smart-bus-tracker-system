...
// Calculate distance between bus and the user
...
const BusDetailSheet = ({ bus, userLocation, onClose, onTrack }) => {
  ...
  // Calculate the distance & location Name
  useEffect(() => {
    if (!bus) return;
    ...
    if (userLocation) {
      ...
      setDistanceAway(dist.toFixed(1));
    }

    // Use the 'nextStop' or 'currentRoad' if available
    if (bus.nextStop) {
    ...
    <div className="absolute inset-x-0 bottom-0 z-50 bg-white rounded-t-[2.5rem] shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.2)] animate-slide-up overflow-hidden">
      ...
      <div className="p-6 relative z-10">
        ...
        <Button
          onClick={() => onTrack(bus)}
          variant="primary"
          className="w-full py-5 text-lg shadow-blue-500/40"
        >
